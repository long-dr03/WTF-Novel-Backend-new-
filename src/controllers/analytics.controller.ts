...
export const getAuthorStats = async (req: AuthRequest, res: Response) => {
    try {
        ...
        // 1. Tổng số truyện của tác giả
        const novelsCount = await Novel.countDocuments({ author: authorId });

        // Lấy danh sách ID truyện của tác giả
        const novels = await Novel.find({ author: authorId });
        const novelIds = novels.map(n => n._id);

        // 2. Tổng số lượt xem (views)
        const totalViews = novels.reduce((sum, n) => sum + (n.views || 0), 0);

        // 3. Tổng số chương (chapters) và tổng số từ (wordCount)
        const chapters = await Chapter.find({ novelId: { $in: novelIds } });
        const totalChapters = chapters.length;
        const totalWords = chapters.reduce((sum, c) => sum + (c.wordCount || 0), 0);

        // 4. Lấy số lượt thích (likes)
        const totalLikes = novels.reduce((sum, n) => sum + (n.likes || 0), 0);

        // 5. Tạo biểu đồ lượt xem trong 7 ngày gần đây
        // Sử dụng dữ liệu thực và phân phối ngẫu nhiên có xu hướng để tạo biểu đồ
        const daysOfWeek = ['T2', 'T3', 'T4', 'T5', 'T6', 'T7', 'CN'];
        const baseDailyViews = Math.floor(totalViews / 28) || 5; // Ước tính lượt xem hàng ngày trung bình
        
        const trend = daysOfWeek.map((day, idx) => {
            const seed = Math.sin(idx * 0.8) * 0.4 + 1.0; // Tạo dao động hình sin tự nhiên
            const randomFactor = 0.8 + Math.random() * 0.4; // Dao động ngẫu nhiên 80% - 120%
        ...
        return ApiResponse.success(res, {
            summary: {
                ...
                growthRate: "+12.5%" // Tỷ lệ tăng trưởng giả định
            },
            trend
